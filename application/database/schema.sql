...
--
-- Permissions: the app knows exactly two user types, stored directly
-- on the user record (no roles/permissions tables):
--   admin          -> user_type = 'admin', warehouse_id = NULL (global access)
--   user_warehouse -> user_type = 'user_warehouse', warehouse_id = their single
--                     warehouse. The warehouse_id FK is added after the
--                     warehouses table is created below.
...
-- ============================================================
-- Users -> warehouses relationship
-- ============================================================
-- users is created before warehouses, so the FK is added here. The
-- guard keeps re-runs of this script idempotent. Deleting a warehouse
-- only NULLs the user's assignment (it never cascades to the user).

SET @users_warehouse_fk_exists := (
    SELECT COUNT(*)
    FROM information_schema.TABLE_CONSTRAINTS
    WHERE CONSTRAINT_SCHEMA = DATABASE()
      AND TABLE_NAME = 'users'
      AND CONSTRAINT_NAME = 'fk_users_warehouse_id'
);
SET @users_warehouse_fk_sql := IF(@users_warehouse_fk_exists = 0,
    ...
    'SELECT 1');
PREPARE users_warehouse_fk_stmt FROM @users_warehouse_fk_sql;
EXECUTE users_warehouse_fk_stmt;
DEALLOCATE PREPARE users_warehouse_fk_stmt;
...
-- ============================================================
-- Seed data: sample users (one admin, one warehouse user)
-- ============================================================
-- The warehouse user is resolved by warehouse name so the seed survives
-- re-insertion with different warehouse IDs. Usernames are unique, so
-- INSERT IGNORE keeps re-runs idempotent.
-- Passwords: admin / admin123, warehouse1 / warehouse123.

INSERT IGNORE INTO users (username, password, user_type, warehouse_id) VALUES
    ('admin',      '$2y$12$Qodu.dMl2Cf5Fa.W.hw.ZOlIS3IFSXKsSToDAtr9BN9EUQizPwTrW', 'admin',          NULL),
    ('warehouse1', '$2y$12$32RvemgIflaFK3z/HKchJuqmkJP6tchJJ2BztBYai50BblB9Mrs9u', 'user_warehouse', (SELECT id FROM warehouses WHERE name = 'Main Warehouse'));
